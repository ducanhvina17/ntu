...
void frontBackSplitLinkedList(LinkedList* ll, LinkedList * resultFrontList, LinkedList *resultBackList)
{
	...
	// Clear front and back list if they're not null
	if (resultBackList->size > 0)
		removeAllItems(resultBackList);
	
	if (resultFrontList->size > 0)
		removeAllItems(resultFrontList);

	// Set current node to first node
	cur = ll->head;

	// If size of LinkedList is odd, extra element goes to the front list
	f = ll->size % 2 == 0 ? ll->size / 2 : ll->size / 2 + 1;
	b = ll->size / 2;

	// Add items to the front list
	while (f > 0)
	{
