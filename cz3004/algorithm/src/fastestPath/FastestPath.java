...
public class FastestPath {
	...
	private static final String FASTEST_PATH = "fastest_path.txt";
	...
	/**
	 * Display map with fastest path.
	 */
	private void DisplayFastestPath() {
		System.out.println("\nFastest path: ");
		...
		for (int i = 0; i < 20; ++i) {
			for (int j = 0; j < 15; ++j) {
				if (i == si && j == sj)
					...
					System.out.print("E ");
				else if (grid[i][j].details == 2)
					System.out.print("B ");
				else if (grid[i][j].details == 0)
					System.out.print("0 ");
				else if (grid[i][j].details == 8)
					System.out.print("* ");
				else
					System.out.print("1 ");
			}
	 ...
	 *            end point column.
	 */
	public void FindFastestPath(int si, int sj, int ei, int ej) {
		this.si = si;
		this.sj = sj;
		this.ei = ei;
		this.ej = ej;
		...
		DisplayFastestPath();
		FileRW.storeFastestPathMap(grid, FASTEST_PATH);
	}
